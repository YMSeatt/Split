...
APP_VERSION = "v53.0" # Version incremented
...
class CalendarPopup(simpledialog.Dialog):
    ...
    '''
    def __init__(self, parent, date_variable):
        self.date_variable = date_variable
        super().__init__(parent, "Select Date")

    def body(self, master):
        self.cal = Calendar(master, firstweekday=calendar.SUNDAY)
        self.cal.pack(expand=True, fill="both")
        self.minsize(255, 184)
        return self.cal

    def apply(self):
        selected_date = self.cal.selection
        if selected_date:
            self.date_variable.set(selected_date.strftime('%Y-%m-%d'))
...
class Calendar(ttk.Frame):
    ...
    def __minsize(self, evt):
        width2, height2 = self._calendar.master.winfo_width(), self._calendar.master.winfo_height()
        print(width2, height2)
        #height = #height[:height.index('+')]
        #self.ms.wm_minsize(self=self.master, width=width2, height=height2)
...
def show_cal_start(self, master):
    if not self.start_date_var: self.start_date_var = tk.StringVar
    self.calendar_start = CalendarPopup(master, self.start_date_var)
    return self.start_date_var
